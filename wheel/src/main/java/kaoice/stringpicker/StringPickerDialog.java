...
public class StringPickerDialog extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        final StringPicker stringPicker2 = (StringPicker) view.findViewById(R.id.string_picker2);
        final Bundle params = getArguments();
        if (params == null) {
            throw new RuntimeException("params is null!");
        }

        final DataModel values = params.getParcelable(getValue(R.string.string_picker_dialog_values));
        stringPicker.setValues(values.getFirst());
        final HashMap secondData=values.getSecond();
        ...
        builder.setPositiveButton(getValue(R.string.string_picker_dialog_ok), new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                mListener.onClick(stringPicker.getCurrentValue(), stringPicker2.getCurrentValue());
            }
        });
